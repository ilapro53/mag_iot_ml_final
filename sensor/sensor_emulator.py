"""
Эмулятор тепличной фермы — 4 датчика + ПОГОДА, публикация в MQTT-брокер.
...
Данные реалистичны и СВЯЗАНЫ между собой:
  ...
  - ПОГОДА меняется каждые 8 модельных часов, ходит по кругу
        Облачно с прояснениями <-> Ясно <-> Пасмурно <-> Дождь <-> (замыкается),
    шаг только на соседнее состояние: вправо 75% / влево 25%.
    Влияет на температуру и освещённость (переходы плавные);
  - CO2 зависит от освещённости (больше света -> меньше CO2, фотосинтез);
...
P_FORWARD = 0.50        # вероятность шага ВПЕРЁД по кругу погоды
...
class Weather:
    ...
    light_mult: float   # множитель к освещённости
    hum_add: float = 0.0  # добавка к влажности, % (дождь -> к аномалии)


# Круг погоды (вперёд), 6 позиций: Облачно -> Пасмурно -> Дождь -> Пасмурно -> Облачно -> Ясно -> (замыкается).
# Пасмурно и Облачно повторяются по бокам — к Дождю/Ясно погода приходит постепенно.
WEATHER = [
    Weather("⛅ Облачно с прояснениями", 1.00, 1.00,  0.0),   # 0
    Weather("☁️ Пасмурно",              0.95, 0.90,  5.0),   # 1  темп -5%, свет -10%, влажнее
    Weather("🌧️ Дождь",                 0.85, 0.39, 12.0),   # 2  темп -15%, свет ×0.39 (полдень ≈ 4 утра при ⛅), влажность к аномалии
...
SENSORS = [
    #              name           unit   night     day      sigma   lo      hi       interval
    SensorConfig("temperature", "°C",   21.5,     24.5,     0.25,   10.0,   35.0,    2.0),
    SensorConfig("co2",         "ppm",  1450.0,   720.0,    15.0,   380.0,  1500.0,  5.0),   # 1450=темно(ночь) .. 720=светло(день)
...
# текущие значения датчиков — для связей (температура<-влажность, CO2<-свет)
current = {s.name: (s.night + s.day) / 2.0 for s in SENSORS}
current_lock = threading.Lock()

START = time.time()           # момент старта (переопределяется в main())
START_TOD_H = 0.0             # реальное время суток (часы 0..24) на старте — задаётся в main()
...
def daylight() -> float:
    """Освещённость суток 0..1: 0 — полночь, 1 — полдень (плавно)."""
...
def weather_mults():
    ...
    ha_target = c.hum_add    # мгновенная цель: динамику влажности (быстро вверх / медленно вниз) задаёт датчик
...
def baseline(cfg: SensorConfig, d: float, tm: float, lm: float, ha: float, cur: dict) -> float:
    ...
    if cfg.name == "temperature":
        ...
        return dn * tm - 0.15 * (hum - 66.0)             # погода + влияние влажности
    if cfg.name == "co2":
        # CO2 зависит от УРОВНЯ освещённости (база без шума -> чистое разделение погод),
        # круто при низком свете: дождь ночью (очень темно) -> часто >1200; пасмурно ночью -> иногда;
        # ясно / облачно с прояснениями -> никогда (светлее).
        light_base = (8000.0 + 34000.0 * d) * lm         # база освещённости (как у датчика light) * погода
...
def connect(client_id: str) -> mqtt_client.Client:
    """Подключение к нашему брокеру с авторизацией и авто-reconnect; ждём готовности брокера."""
...
def weather_controller():
    """Каждые WEATHER_PERIOD_H модельных часов: вперёд 50% / остаться 30% / назад 20%."""
    ...
    print(f"  [weather] старт: {WEATHER[0].name}; смена каждые {WEATHER_PERIOD_H:.0f} мод.ч "
          f"(~{period_real:.1f} с реального)", flush=True)
    while not stop_flag.is_set():
        ...
        if step == 0:
            ...
            print(f"  [weather] остаётся {name}", flush=True)
            continue
        with weather_lock:
            ...
            name = WEATHER[weather_state["cur"]].name
        print(f"  [weather] -> {name} ({'вперёд' if step > 0 else 'назад'})", flush=True)
...
def publisher(cfg: SensorConfig):
    ...
    n = 0
    print(f"  [pub:{cfg.name}] старт -> {cfg.topic} каждые {cfg.interval} с", flush=True)
    try:
        while not stop_flag.is_set():
            n += 1
            tm, lm, ha_target, wname = weather_mults()
            if cfg.name == "humidity":
                # влажность: быстрый подъём при дожде, медленный спад после
            ...
            print(f"  pub {cfg.name:<11} = {value} {cfg.unit} (#{n}) [{wname}]{mark}", flush=True)
...
def main():
    ...
    print(f"=== Эмулятор фермы -> {MQTT_HOST}:{MQTT_PORT} (user={MQTT_USER}) ===", flush=True)
