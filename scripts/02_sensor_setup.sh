...
if [ -z "${DAY_PERIOD_SEC:-}" ] && [ -z "${RATE_MULT:-}" ] && [ -z "${MODE:-}" ] && [ -t 0 ]; then
    ...
    echo "  2) fast    - ускоренный, сутки за ~19 секунд (для Python-визуализатора)"
    printf "Номер [1]: "
    read -r _ans
    case "$_ans" in
...
case "${MODE:-normal}" in
    fast) DAY_PERIOD_SEC="${DAY_PERIOD_SEC:-18.75}"; RATE_MULT="${RATE_MULT:-8}" ;;
...
# Длина модельных суток в секундах: 86400 = реальное время (24 ч), 18.75 = сутки за ~19 секунд (fast).
DAY_PERIOD_SEC=${DAY_PERIOD_SEC}
# Частота публикаций: реальное время -> 0.02 (гладко по правилу 1800/DAY_PERIOD), fast -> 8.
RATE_MULT=${RATE_MULT}
TZ=Europe/Moscow
EOF
...
echo " Режим времени: DAY_PERIOD_SEC=${DAY_PERIOD_SEC} (86400 = реальное, 18.75 = fast)."
