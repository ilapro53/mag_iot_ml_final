...
|--------|-----------|----------|
| `.env.fast` | 18.75 c (сутки за ~19 с) | Python-графики, циклы видно сразу |
...
- **fast** - ускоренный, сутки за ~19 секунд, пресет `.env.fast`, для Python-визуализатора (`DAY_PERIOD_SEC=18.75`, `RATE_MULT=8`).
...
Частоту держи по правилу `RATE_MULT = 1800 / DAY_PERIOD_SEC` (реальное время -> `0.02`, fast -> `8`),
